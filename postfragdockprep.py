...
def main():
    ...
    if args.mode == 'prep':
        inputmap = prepareinputs(inputmap,args.sizecut,args.endcut)
        inputmap = allfragrms(args,inputmap)
        writeall(args,inputmap)
    if args.mode == 'stats':
        inputmap = prepareinputs(inputmap,args.sizecut,args.endcut)
        inputmap = allfragrms(args,inputmap)
...
def parseargs():
    ...
    parser.add_argument('-ec','--endcut',type=int,default=0,help='Cut the terminal residues off each end of the fragments (The end residues of often problematic in fragdocking)')
    parser.add_argument('-go','--goodonly',type=bool,default=False,help='Only grab the fragments under 2.5 RMS')
...
def fragrms(frag,native):
    ...
    for atom in frag.atoms:
        ...
        #print "distance between " + str(atom.resnum) + " and " + str(natatom.resnum) + " is " + str(dist)
        totaldist += dist*dist
    rms = math.sqrt(totaldist/fragCAcount(frag))
    return rms
...
def allfragrms(args,pdbmap):
    for pdb in pdbmap.keys():
        ...
        for native in args.natives:
            ...
            if pdbkey in native or len(pdbmap.keys()) ==1:
                it = 0
        ...
        for fragment in pdbmap[pdb]:
            ...
            if rms > 2.0:
                if len(fragment.atoms) > longestbadfraglen:
                    longestbadfraglen = fragCAcount(fragment)
...
def writepdbfromfrags(args,fraglist,name):
    newpdb = []
    lfraglen = 0
...
def prepareinputs(inputmap,sizecut,endcut):
    for pdb in inputmap.keys():
        ...
        for frag in frags:
            ...
            if rescount >= sizecut:
                newatoms = []
                start = frag.atoms[0].resnum+endcut
                end = frag.atoms[-1].resnum-endcut
                for atom in frag.atoms:
                    if atom.resnum >= start and atom.resnum <= end:
                        newatoms.append(atom)
                print "appending " + str(len(newatoms)) + " atoms"
                cutfrag = Fragment(newatoms)
                keptfrags.append(cutfrag)
...
def writeall(args,inputmap):
    for pdb in inputmap.keys():
        frags = inputmap[pdb]
        pdbkey = re.split('_',pdb)[0]
        writepdbfromfrags(args,frags,'growerinput.pdb')
